...
|        | **查看某一个作业的提交情况**   | `workId`: string | `title`:string <br>`list`:Array 每一项包含`userId`:string<br>`name`: string<br>`status`:string(unsubmitted或者submitted或者graded) <br>`grade`:number<br>`content`:string<br>`teacherComment`:string(教师评语)    | `/homework/submissions:workId`                | **GET**    |
