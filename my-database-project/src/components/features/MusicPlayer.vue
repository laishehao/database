<template>
  <div class="music-app-container">
    ...
    <transition name="music-super-pop">
      <!-- 
        使用 v-if 确保关闭时彻底销毁 iframe，从而停止音乐播放
      -->
      <div v-if="isExpanded" class="premium-player-card">
        ...
        <div class="player-body">
          ...
          <div class="player-meta">
            <div class="scrolling-title">
              <span class="text">正在播放您的专属歌单 - 悦动心弦 (网易云音乐)</span>
            </div>
        ...
        <div class="iframe-glass-slot">
          <!-- 
            优化点：
            1. height 增加到 110，防止底部控制栏和暂停键被截断
            2. 移除 scrolling="no" 以防万一，但通常网易云外链是自适应的
          -->
          <iframe 
            frameborder="no" 
            border="0" 
            ...
            height="110" 
            allow="autoplay"
            src="https://music.163.com/outchain/player?type=0&id=308300106&auto=1&height=90">
          </iframe>
        </div>
...
.premium-player-card {
  ...
  padding: 20px 20px 25px 20px; /* 增加底部内边距，确保 footer 完整 */
  overflow: hidden;
...
/* Iframe 槽位优化：增加高度，取消严格的 overflow: hidden */
.iframe-glass-slot {
  background: rgba(255, 255, 255, 0.4);
  border-radius: 12px;
  padding: 0;
