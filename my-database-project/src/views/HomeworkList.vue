      ...
      <div v-if="hasHomework" class="cute-table-wrapper">
        ...
        >
          ...
          <el-table-column label="🍓 状态" width="120">
            <template slot-scope="scope">
              <div :class="['status-badge', scope.row.progress < 100 ? 'status-ing' : 'status-done']">
          ...
          <el-table-column label="💖 操作" width="200" fixed="right">
            <template slot-scope="scope">
              <el-button type="text" size="small" class="action-btn btn-grade" @click="handleGrade(scope.row)">
                批改
              </el-button>
              <el-button type="text" size="small" class="action-btn btn-edit" @click="handleEdit(scope.row)">
                编辑
              </el-button>
              <el-button type="text" size="small" class="action-btn btn-delete" @click="handleDelete(scope.row)">
                删除
              </el-button>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(`要让 "${row.title}" 消失嘛？(｡•́︿•̀｡)`, "这是个严肃的提示", {
...
.cute-text-title { font-weight: bold; color: #6B4C56; }
.cute-tag { background-color: #E0F7FA; border-color: #B2EBF2; color: #00BCD4; border-radius: 12px; font-weight: bold; }

.progress-box { display: flex; align-items: center; }
.cute-progress { flex: 1; margin-right: 8px; }
.progress-txt { font-size: 12px; font-weight: bold; color: #FF69B4; background: #FFF0F5; padding: 2px 6px; border-radius: 10px; }
::v-deep .el-progress-bar__outer, ::v-deep .el-progress-bar__inner { border-radius: 10px; }

.status-badge { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: bold; }
.status-ing { background-color: #FFF3E0; color: #FF9800; border: 1px solid #FFE0B2; }
.status-done { background-color: #E8F5E9; color: #4CAF50; border: 1px solid #C8E6C9; }

.action-btn { font-weight: bold; border-radius: 12px; padding: 5px 10px; transition: all 0.2s; }
.action-btn:hover { background-color: #FFF0F5; }
.btn-grade { color: #87CEEB; }
.btn-edit { color: #DDA0DD; }
.btn-delete { color: #FFB7C5; }
