...
def app_register():
    ...
    if if_ok["msg"]!="注册成功":
        data_500 = jsonify({"msg": if_ok["msg"], "status": "fail"})
        data_500.status_code = 500
        abort(data_500)
...
def app_userinfo():
    ...
    if gender=="male":
        ...
        gender="女"
    logger.info(f'phone={phone},name={name},email={email},role={role},id={id},gender={gender},password={password},major={major}')
    
    if_ok= userinfo (id,phone, name, email,role,gender,password,major)
    logger.info(f'if_ok={if_ok["msg"]}')
...
def app_add_student():
    ...
    major=data.get('major')
    phone = data.get('phone')   
    Cno=data.get('courseId')
    Sno=data.get('studentId')
    logger.info(f'studentId={studentId},name={name},gender={gender},major={major},phone={phone},Cno={Cno},Sno={Sno}')
...
def app_delete_student(studentId):
    ...
    logger_py.info(f'studentId={studentId},Cno={Cno}')
...
def app_add_work():
    ...
    if_ok= add_work (title,course,content,progress,userId,starttime,overtime)
    logger.info(f'if_ok={if_ok["msg"]}')
    return if_ok
# TODO: 老师作业删除
...
def app_submit_work(workId):
    ...
    if_ok= submit_work (role,syudentId,workId,content)
    return if_ok
# TODO: 学生做作业界面
